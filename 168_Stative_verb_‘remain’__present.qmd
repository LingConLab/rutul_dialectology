---
title: "Stative verb ‘remain’: present"
...
Stative verbs may have either a synthetic or periphrastic present. The synthetic form is *ma<b>a* (Gender 3). The periphrastic form consists of the converbial form of the stative and the present-tense copula (e.g. *ma<b>a-na w-iʔi* ‘remain<3>-CVB 3-COP’ in Kina). In some varieties, only the periphrastic form is used.
...
lexeme_data <- db |> 
  filter(feature_id == 168) |>  # First filter by feature_id
  filter(feature_lexeme == "Gender 1") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |> 
  filter(feature_id == 168) |>  # First filter by feature_id
  filter(feature_lexeme == "Gender 2") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
...
lexeme_data <- db |> 
  filter(feature_id == 168) |>  # First filter by feature_id
  filter(feature_lexeme == "Gender 3") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
...
lexeme_data <- db |> 
  filter(feature_id == 168) |>  # First filter by feature_id
  filter(feature_lexeme == "Gender 4") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
