...
## PROX (mi-) {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'PROX (mi-)')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
...
## MED (ha-) {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'MED (ha-)')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
  mutate(value = str_split(value, ' ; ')) |> 
  unnest_longer(value) |> 
...
## DIST (ti-/či-) {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'DIST (ti-/či-)')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
  mutate(value = str_split(value, ' ; ')) |> 
  unnest_longer(value) |> 
