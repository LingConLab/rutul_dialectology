...
The singular absolutive of the noun ‘goat’ is either *cʼihʲ* or *cʼij* (see [feature](https://lingconlab.github.io/rutul_dialectology/028_Phonetic_variation_in_the_lexical_item_%E2%80%98goat%E2%80%99.html)). The plural absolutive can be either regular: *cʼiher*, *cʼihe* and *cʼijmar* (and their phonetic variants) or irregular: *cʼuhuler* (and its phonetic variants). Depending on the village, in our data only the regular forms are attested, only irregular ones, or both types of forms may coexist.
...
feature_data <- db |> 
  filter(feature_id == 48) |>  # Filter by feature_id
