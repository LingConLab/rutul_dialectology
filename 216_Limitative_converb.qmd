...
There are a number of ways in which the limitative semantics may be expressed ("until X happens"). The most common strategy is a specialized converbial suffix attached directly to the verbal stem; however, some other strategies occur as well. Some suffixes with limitative meaning are found to be able to attach not only to verbal stems, but also to infinitives or general converbs.
...
lexeme_data <- db |> 
  filter(feature_id == 216) |>  # First filter by feature_id
  filter(feature_lexeme == "PFV") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |> 
  filter(feature_id == 216) |>  # First filter by feature_id
  filter(feature_lexeme == "IPFV") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
