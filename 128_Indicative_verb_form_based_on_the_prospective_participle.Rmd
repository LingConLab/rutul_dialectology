---
title: Indicative verb form based on the prospective participle
author: Timur Maisak
...
In the verb paradigm of most Rutul varieties, there is a form based on the prospective (future) participle. In the affirmative, it employs the copula as an auxiliary verb; in the negative, it employs the negative copula. The form expresses prospective or debitive future, e.g. zad bɨga zer wezasdɨ i 'Tomorrow I am going (or: I have to) milk the cow' (Luchek) refers to the speaker's intention or obligation. The map shows whether the form based on the prospective participle was accepted by the speakers.


## Affirmative form: INF.ATTR + COP {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'Affirmative form: INF.ATTR + COP')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
...
## Negative form: INF.ATTR + COP.NEG {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'Negative form: INF.ATTR + COP.NEG')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
  mutate(value = str_split(value, ' ; ')) |> 
  unnest_longer(value) |> 
