---
title: "Suffix -d(V) in demonstratives"
...
lexeme_data <- db |> 
  filter(feature_id == 206) |>  # First filter by feature_id
  filter(feature_lexeme == "Proximal (*mi*-)") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |> 
  filter(feature_id == 206) |>  # First filter by feature_id
  filter(feature_lexeme == "Medial (*ha*-)") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
...
lexeme_data <- db |> 
  filter(feature_id == 206) |>  # First filter by feature_id
  filter(feature_lexeme == "Distal (*ti*-/*či*-)") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
