...
## Affirmative form: IPFV.ATTR + COP
...
lexeme_data <- db |> 
  filter(feature_id == 138) |>  # First filter by feature_id
  filter(feature_lexeme == "Affirmative form: IPFV.ATTR + COP") |>  # Then filter by feature_lexeme
...
## Negative form: PFV.ATTR + COP.NEG
...
lexeme_data <- db |> 
  filter(feature_id == 138) |>  # First filter by feature_id
  filter(feature_lexeme == "Negative form: PFV.ATTR + COP.NEG") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
