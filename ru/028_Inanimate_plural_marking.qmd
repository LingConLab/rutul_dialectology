...
We elicited three contexts: ‘They cut down the trees’, ‘They built houses for themselves’, ‘They brought axes’. The maps show the plural marking on the nouns ‘tree’, ‘house’ and ‘axe’. The attested variants include: *-bɨ* (attested only in Khnov) and *-bɨr* (elsewhere).
...
lexeme_data <- db |>
  filter(feature_id == 28) |>  # First filter by feature_id
  filter(feature_lexeme == "‘tree’") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |>
  filter(feature_id == 28) |>  # First filter by feature_id
  filter(feature_lexeme == "‘house’") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
...
lexeme_data <- db |>
  filter(feature_id == 28) |>  # First filter by feature_id
  filter(feature_lexeme == "‘axe’") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
