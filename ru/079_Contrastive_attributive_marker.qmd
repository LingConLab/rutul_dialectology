---
title: "Contrastive attributive marker"
author: "Ilya Sadakov"
date: 2025-08-28
format: html
lang: ru
...
lexeme_data <- db |>
  filter(feature_id == 79) |>  # First filter by feature_id
  filter(feature_lexeme == "‘Older brother’ vs ‘younger brother’") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |>
  filter(feature_id == 79) |>  # First filter by feature_id
  filter(feature_lexeme == "‘The one’ vs ‘the other’") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
...
lexeme_data <- db |>
  filter(feature_id == 79) |>  # First filter by feature_id
  filter(feature_lexeme == "‘Exactly the white cow’") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
