...
Most indicative verb forms in Rutul are periphrastic and include a lexical verb and a postposed auxiliary. In most varieties, the negative future form consists of the infinitive and the negative copula diš (e.g. *wezas-diš* ‘will not milk’). In Shinaz, a fusion /sǯ/ > /č/ took place (e.g. *wezečiš* ‘will not milk’).
...
feature_data <- db |> 
  filter(feature_id == 148) |>  # Filter by feature_id
