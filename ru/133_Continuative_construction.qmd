...
lexeme_data <- db |>
  filter(feature_id == 133) |>  # First filter by feature_id
  filter(feature_lexeme == "Presence of the construction") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |>
  filter(feature_id == 133) |>  # First filter by feature_id
  filter(feature_lexeme == "Gender agreement in the auxiliary verb") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
