---
title: "Oblique stem of the numeral ‘two’"
author: "Ilya Sadakov"
date: 2025-08-28
format: html
lang: ru
...
The maps show the forms of oblique stem of the numeral ‘two’ in the contexts ‘One did, and **two** did not’ and ‘One cow follows **two** (cows)’. The form *qʼuˤn*- is attested everywhere.
...
lexeme_data <- db |>
  filter(feature_id == 93) |>  # First filter by feature_id
  filter(feature_lexeme == "two.ERG") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |>
  filter(feature_id == 93) |>  # First filter by feature_id
  filter(feature_lexeme == "two.ATTR") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
