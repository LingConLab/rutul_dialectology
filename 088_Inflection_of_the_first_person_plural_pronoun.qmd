...
lexeme_data <- db |> 
  filter(feature_id == 88) |>  # First filter by feature_id
  filter(feature_lexeme == "we.ABS") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |> 
  filter(feature_id == 88) |>  # First filter by feature_id
  filter(feature_lexeme == "we.ERG") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
