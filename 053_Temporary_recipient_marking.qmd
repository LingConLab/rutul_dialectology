---
title: "Temporary recipient marking"
...
We elicited the context ‘Give me your hand’ and ‘Give me back (my) knife’. The maps show the case marking on the pronoun ‘me’ in these contexts. The attested variants include: *-da* / *-dda* / *-dɨ* / *-d* and *-s* (attested only in Dzhilikhur).
...
lexeme_data <- db |> 
  filter(feature_id == 53) |>  # First filter by feature_id
  filter(feature_lexeme == "‘Give me back (my) knife’") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |> 
  filter(feature_id == 53) |>  # First filter by feature_id
  filter(feature_lexeme == "‘Give me your hand’") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
