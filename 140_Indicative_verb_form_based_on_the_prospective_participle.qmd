...
lexeme_data <- db |> 
  filter(feature_id == 140) |>  # First filter by feature_id
  filter(feature_lexeme == "Affirmative form: INF.ATTR + COP") |>  # Then filter by feature_lexeme
...
## Negative form: INF.ATTR + COP.NEG
...
lexeme_data <- db |> 
  filter(feature_id == 140) |>  # First filter by feature_id
  filter(feature_lexeme == "Negative form: INF.ATTR + COP.NEG") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
