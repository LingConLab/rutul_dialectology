...
date: 'Last update: `r lubridate::make_datetime(year = 2023, month = 9, day = 21)`'
output:
  html_document:
...
In some dialects, certain verbs show vowel fronting when the S/P argument is an animate (as opposed to inanimate, i.e. humans and animals) plural. In the verbs on the map, the root vowel -u- of the singular and inanimate plural may alternate with -i-, -ü- or -ɨ- in the animate plural. In the same verbs, the vowel following the root may change from -a- or -u- to  -e-, -i-, -ɨ- or -ä-. Verbs with a~e fronting of the root vowel are not included.
...
## playAPLroot {.tabset .tabset-fade .tabset-pills}
...
## playAPLstem {.tabset .tabset-fade .tabset-pills}
...
## sitAPLroot {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'sitAPLroot')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
  mutate(value = str_split(value, ' ; ')) |> 
  unnest_longer(value) |> 
...
## sitAPLstem {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'sitAPLstem')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
  mutate(value = str_split(value, ' ; ')) |> 
  unnest_longer(value) |> 
...
## walkAPLroot {.tabset .tabset-fade .tabset-pills}
...
## walkAPLstem {.tabset .tabset-fade .tabset-pills}
