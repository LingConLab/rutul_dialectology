...
lexeme_data <- db |> 
  filter(feature_id == 31) |>  # First filter by feature_id
  filter(feature_lexeme == "‘from Fatima’s (place)’") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |> 
  filter(feature_id == 31) |>  # First filter by feature_id
  filter(feature_lexeme == "‘from Rasul's (place)’") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
