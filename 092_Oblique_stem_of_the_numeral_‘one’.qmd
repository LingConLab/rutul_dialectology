---
title: "Oblique stem of the numeral ‘one’"
author: "Ilya Sadakov"
date: 2025-08-28
format: html
css: styles.css
...
The maps show the forms of oblique stem of the numeral ‘one’ in the contexts ‘**One** did, and two did not’ and ‘Two apples are better than **one**’. The variants include *sa*- (used almost exclusively in Ikhrek) and *sɨn*- (used elsewhere).
...
lexeme_data <- db |> 
  filter(feature_id == 92) |>  # First filter by feature_id
  filter(feature_lexeme == "one.ERG") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |> 
  filter(feature_id == 92) |>  # First filter by feature_id
  filter(feature_lexeme == "one.SUPEL") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
