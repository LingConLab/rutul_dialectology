---
title: Marking of standard of comparison
...
The maps show the marking of the standard of comparison in comparative constructions. We elicited two contexts: ‘The mother is kinder than the father’ and ‘A pear is sweeter than an apple’. Attested markers are as follows: *-ɨːla* / *-iːla* (and other phonetic variants), *-qa* / *-qaʔ*, *-qana* (attested with only one consultant, in Kiche), *-χda* (attested with only one consultant, in Dzhilikhur).


## ‘father’ {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == '‘father’')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
...
## ‘apple’ {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == '‘apple’')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
  mutate(value = str_split(value, ' ; ')) |> 
  unnest_longer(value) |> 
