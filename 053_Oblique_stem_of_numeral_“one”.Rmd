---
title: Oblique stem of numeral “one”
author: Ilya Sadakov
...
The map shows the form of oblique stem of numeral ‘one’ in the ergative and the superelative.


## one.ERG {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'one.ERG')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
...
## one.SUPEL {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'one.SUPEL')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
  mutate(value = str_split(value, ' ; ')) |> 
  unnest_longer(value) |> 
