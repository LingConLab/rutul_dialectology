---
title: Inflection of the first person plural pronoun
author: Ilya Sadakov
...
Two maps show the absolutive and the ergative forms of the first person plural pronoun 'we'. In languages with exclusive - inclusive distinction, the forms of the exclusive pronouns are mapped.


## we.ABS {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'we.ABS')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
...
## we.ERG {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'we.ERG')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
  mutate(value = str_split(value, ' ; ')) |> 
  unnest_longer(value) |> 
