...
The map shows whether the quotative marker xur (fu in Khnov) isused in reported speech construction. Only the first answer is taken into account. We used Russian stimuli that contained the Russian quotative marker мол.


## he said: come {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'he said: come')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
...
## she says: don’t come {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'she says: don’t come')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
  mutate(value = str_split(value, ' ; ')) |> 
  unnest_longer(value) |> 
