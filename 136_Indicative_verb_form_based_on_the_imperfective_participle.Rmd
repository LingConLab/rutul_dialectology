---
title: Indicative verb form based on the imperfective participle
author: Timur Maisak
...
In the verb paradigm of most Rutul varieties, there is a form based on the imperfective participle. In the affirmative, it employs the copula as an auxiliary verb; in the negative, it employs the negative copula. The form expresses habitual or generic past, e.g. izdɨ šur bala p'ap'rɨs deʔed-i 'My brother smokes a lot' (Luchek) refers to the brother's habit. The map shows whether the form based on the imperfective participle was accepted by the speakers. Note that in affirmative forms, the attributive marker -d before the copula can optionally shift to -t' in some varieties (Kina, Luchek, Kala, Kiche).


## Affirmative form: IPFV.ATTR + COP {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'Affirmative form: IPFV.ATTR + COP')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
...
## Negative form: PFV.ATTR + COP.NEG {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == 'Negative form: PFV.ATTR + COP.NEG')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
  mutate(value = str_split(value, ' ; ')) |> 
  unnest_longer(value) |> 
