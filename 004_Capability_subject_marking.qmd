---
title: "Capability subject marking"
...
We elicited two contexts with capability subjects: ‘My brother can swim across the river’ (affirmative) and ‘Rasul couldn't lift this stone’ (negative). The maps show the markers used on the capability subjects in these contexts, including *-daː*, *-dːa* and *-da* (attested with only one consultant, in Amsar).
...
lexeme_data <- db |> 
  filter(feature_id == 4) |>  # First filter by feature_id
  filter(feature_lexeme == "AFFIRMATIVE") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |> 
  filter(feature_id == 4) |>  # First filter by feature_id
  filter(feature_lexeme == "NEGATIVE") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
