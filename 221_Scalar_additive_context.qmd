...
Scalar additive semantics ("even X did Y", where X is the least likely element to do Y) is usually expressed by an additive particle. However, in some cases, the additive is required to combine with another element, -kina/-kine, in other to convey scalar additive meaning (and not just additive one).
...
feature_data <- db |> 
  filter(feature_id == 221) |>  # Filter by feature_id
