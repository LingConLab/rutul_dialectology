---
title: Inanimate plural marking
...
We elicited three contexts: ‘They cut down the trees’, ‘They built houses for themselves’, ‘They brought axes’. The maps show the plural marking on the nouns ‘tree’, ‘house’ and ‘axe’. The attested variants include: *-bɨ* (attested only in Khnov) and *-bɨ* (elsewhere).
...
## ‘house’ {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == '‘house’')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
  mutate(value = str_split(value, ' ; ')) |> 
  unnest_longer(value) |> 
...
## ‘axe’ {.tabset .tabset-fade .tabset-pills}

### Map

```{r}
db |> 
  filter(feature_lexeme == '‘axe’')  |>
  filter(!is.na(value),
         value != 'NO DATA') |>
  mutate(value = str_split(value, ' ; ')) |> 
  unnest_longer(value) |> 
