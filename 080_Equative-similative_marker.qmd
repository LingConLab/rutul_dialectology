...
lexeme_data <- db |> 
  filter(feature_id == 80) |>  # First filter by feature_id
  filter(feature_lexeme == "Equative: ‘as strong as a bull’") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |> 
  filter(feature_id == 80) |>  # First filter by feature_id
  filter(feature_lexeme == "Similative: ‘walks like a bear’") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
