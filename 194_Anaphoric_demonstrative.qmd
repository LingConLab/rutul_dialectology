...
lexeme_data <- db |> 
  filter(feature_id == 194) |>  # First filter by feature_id
  filter(feature_lexeme == "3rd person singular, human") |>  # Then filter by feature_lexeme
...
lexeme_data <- db |> 
  filter(feature_id == 194) |>  # First filter by feature_id
  filter(feature_lexeme == "3rd person singular, non-human") |>  # Then filter by feature_lexeme
  filter(!is.na(value), value != "NO DATA")
