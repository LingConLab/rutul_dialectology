...
In the present tense, the negative copula has an invariable suppletive form, which does not inflect for gender. It is *diš* in the majority of varieties; and only in Shinaz a phonological shift to *ǯiš* took place.
...
lexeme_data <- db |> 
  filter(feature_id == 147) |>  # First filter by feature_id
  filter(feature_lexeme == "Gender 1/4") |>  # Then filter by feature_lexeme
